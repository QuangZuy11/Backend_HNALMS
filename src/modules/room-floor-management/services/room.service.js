...
exports.createRoom = async (data) => {
  ...
  if (existingCode) throw { status: 400, message: `Mã phòng ${roomCode} đã tồn tại!` };
  const newRoom = new Room({ name, roomCode, floorId, roomTypeId, description, status: "Available", isActive: isActive !== undefined ? isActive : true });
...
exports.updateRoom = async (roomId, updates) => {
  ...
  if (room.status === "Occupied" && (updates.name || updates.roomCode || updates.floorId || updates.roomTypeId)) {
    throw { status: 403, message: "Phòng đang có khách (Occupied). Không được phép thay đổi cấu trúc." };
  }
  if (updates.name && updates.name !== room.name) {
    const duplicateName = await Room.findOne({ name: updates.name });
    if (duplicateName) throw { status: 400, message: "Tên phòng mới bị trùng!" };
  }
...
exports.getAllRooms = async (filters) => {
  ...
  return await Room.find(query).populate("floorId", "name").populate("roomTypeId", "typeName currentPrice images").sort({ name: 1 });
};

exports.getRoomDetail = async (roomId) => {
  const room = await Room.findById(roomId).populate("floorId", "name").populate("roomTypeId", "typeName currentPrice description images");
  if (!room) throw { status: 404, message: "Không tìm thấy phòng" };
  const roomAssets = await RoomDevice.find({ roomTypeId: room.roomTypeId._id }).populate("deviceId", "name brand model type");
...
exports.deleteRoom = async (roomId) => {
  const room = await Room.findById(roomId);
  if (!room) throw { status: 404, message: "Không tìm thấy phòng" };
  if (room.status === "Occupied") throw { status: 403, message: "Phòng đang có khách thuê." };
...
exports.toggleStatus = async (roomId, isActive) => {
  const room = await Room.findById(roomId);
  if (!room) throw { status: 404, message: "Không tìm thấy phòng" };
  if (room.status === "Occupied" && isActive === false) throw { status: 403, message: "Phòng đang có người ở, không thể ngừng hoạt động." };
...
exports.generateTemplateBuffer = async () => {
  const headers = [["Mã phòng", "Tên phòng", "Tên tầng", "Tên loại phòng", "Mô tả"]];
  const wb = xlsx.utils.book_new();
  const ws = xlsx.utils.aoa_to_sheet(headers);
  xlsx.utils.sheet_add_aoa(ws, [["P201", "Phòng 201", "Tầng 2", "Studio", "Gần thang máy"]], { origin: "A2" });
...
exports.importRoomsFromFile = async (file) => {
  ...
  if (!data) throw { status: 500, message: "Lỗi Server: Không đọc được dữ liệu file (Buffer/Path missing)" };
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    if (!roomCode || !name || !floorName || !typeName) {
      errors.push(`Dòng ${rowIndex}: Thiếu thông tin bắt buộc (Mã, Tên, Tầng, Loại).`);
      continue;
    }
    ...
    if (!floorId) {
      errors.push(`Dòng ${rowIndex}: Không tìm thấy tầng có tên "${floorName}".`);
      continue;
    }
    if (!roomTypeId) {
      errors.push(`Dòng ${rowIndex}: Không tìm thấy loại phòng tên "${typeName}".`);
      continue;
    }
  ...
  try {
    ...
    if (dbError.code === 11000) {
       throw { status: 400, message: "Lỗi DB: Một số Mã phòng hoặc Tên phòng trong file đã tồn tại trên hệ thống." };
    }
    throw dbError;
  }
};
