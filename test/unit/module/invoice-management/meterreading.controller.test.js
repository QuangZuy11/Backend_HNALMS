...
describe("MeterReadingController Unit Tests (Controller API logic)", () => {
    ...
    describe("updateReading", () => {
        test("Return 200 (Confirm T) when edit is successful", async () => {
            req.params = { id: "read123" };
            req.body = { newIndex: 180 };
            const mockResult = { _id: "read123", usageAmount: 80 };
            meterReadingService.updateReading.mockResolvedValue(mockResult);

            await MeterReadingController.updateReading(req, res);
        ...
        test("Return 400 with 'Chỉ số mới phải lớn hơn chỉ số cũ' when NewIndex < OldIndex", async () => {
            req.params = { id: "read123" };
            req.body = { newIndex: 50 };
            meterReadingService.updateReading.mockRejectedValue(new Error("Chỉ số mới không được nhỏ hơn chỉ số cũ (Trừ khi đồng hồ quay vòng)"));
